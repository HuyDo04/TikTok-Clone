...
export default function VideoPlayer({ video, isActive, isCommentOpen, setCommentVideoId }) {
  ...
  const toggleComments = () => {
    if (isCommentOpen) {
      ...
      setCommentVideoId(video.id); // Open for this video
    }
  }
  ...
  return (
    <div className={cx("videoPlayerContainer")}>
      <div className={cx("video-content-wrapper")}>
        <video
          ref={videoRef}
          className={cx("video")}
          src={video.videoUrl}
      ...
      {/* Action buttons */}
      <VideoActions video={video} onToggleComments={toggleComments} />

      {isCommentOpen && (
        <CommentModal video={video} onClose={toggleComments} />
      )}
    </div>
  )
}
