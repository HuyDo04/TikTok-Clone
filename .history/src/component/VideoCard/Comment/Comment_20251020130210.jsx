...
export default function Comment({ comment, onReply }) {
  ...
  return (
    <div className={cx("commentWrapper", { open: showReplies })}>
      {/* Comment Item */}
  ...
  <div className={cx('replyContainer', { open: showReplies })}>
    <div className={cx('replyHeader')}>
      ...
      {!showReplies ? (
        ...
        // Danh sách reply khi mở
        <div className={cx('repliesList', { open: showReplies })}>
          {comment.replies.slice(0, visibleCount).map((reply) => (
            <div key={reply.id} className={cx('commentExtend', { open: showReplies })}>
              <div comment={reply} onReply={onReply} />
            </div>
          ))}
