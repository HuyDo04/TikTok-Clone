...
export default function Comment({ comment, onReply, onMention }) {
  ...
  return (
    <div className={styles.comment}>
      <img src={comment.user.avatar || "/placeholder.svg"} alt={comment.user.username} className={styles.avatar} />
      ...
      <div className={styles.content}>
        ...
        <div className={styles.actions}>
          <button className={styles.replyButton} onClick={() => onReply(comment.id, comment.user.username)}>
            Trả lời
          </button>

          {comment.replies && comment.replies.length > 0 && !showReplies && (
            <button className={styles.viewRepliesButton} onClick={() => setShowReplies(true)}>
            ...
            {comment.replies.map((reply) => (
              <div key={reply.id} className={styles.reply}>
                ...
                <div className={styles.replyContent}>
                  ...
                  <button className={styles.replyButton} onClick={() => onReply(comment.id, reply.user.username)}>
                    Trả lời
                  </button>
                </div>
